...
def main():
    ...
    y_test_all_disease = torch.tensor(merged_test_data_all[disease_col].values, dtype=torch.float32).unsqueeze(1)
    y_test_all_confounder = torch.tensor(merged_test_data_all[confounder_col].values, dtype=torch.float32).unsqueeze(1)
    ...
    edge_list = f"Results/MicroKPNN_MT_plots/required_data/EdgeList.csv"
    # Build masks
    ...
    parent_dict_csv_path = "Results/MicroKPNN_MT_plots/required_data/parent_dict_main.csv"
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(X, y_all)):
        ...
        y_all_train_disease = torch.tensor(train_data[disease_col].values, dtype=torch.float32).unsqueeze(1)
        y_all_train_confounder = torch.tensor(train_data[confounder_col].values, dtype=torch.float32).unsqueeze(1)
        x_all_val = torch.tensor(val_data[feature_columns].values, dtype=torch.float32)
        y_all_val_disease = torch.tensor(val_data[disease_col].values, dtype=torch.float32).unsqueeze(1)
        y_all_val_confounder = torch.tensor(val_data[confounder_col].values, dtype=torch.float32).unsqueeze(1)


        # Create stratified DataLoaders.
       
        data_all_loader = create_stratified_dataloader(x_all_train, y_all_train_disease, y_all_train_confounder, train_cfg["batch_size"])
        data_all_val_loader = create_stratified_dataloader(x_all_val, y_all_val_disease, y_all_val_confounder, train_cfg["batch_size"])
        data_all_test_loader = create_stratified_dataloader(x_test_all, y_test_all_disease, y_test_all_confounder, train_cfg["batch_size"])

        # Compute class weights.
        num_pos_disease = y_all_train_disease.sum().item()
        num_neg_disease = len(y_all_train_disease) - num_pos_disease
        ...
        num_pos_drug = y_all_train_confounder.sum().item()
        num_neg_drug = len(y_all_train_confounder) - num_pos_drug
        ...
        optimizer = optim.AdamW(
            list(model.encoder.parameters()) + list(model.disease_classifier.parameters()) + list(model.classifier.parameters()),
        ...
        Results = train_model(
            model, optimizer,
            data_all_loader, data_all_val_loader,
            data_all_test_loader, train_cfg["num_epochs"],
            criterion_classifier, criterion_disease_classifier, device
        )

        # Save model and features.
        torch.save(model.state_dict(), f"Results/MicroKPNN_MT_plots/trained_model_fold{fold+1}.pth")
        pd.Series(feature_columns).to_csv("Results/MicroKPNN_MT_plots/feature_columns.csv", index=False)
        ...
        plot_confusion_matrix(Results["train"]["confusion_matrix"][-1],
                              ...
                              save_path=f"Results/MicroKPNN_MT_plots/fold_{fold+1}_train_conf_matrix.png",
                              class_names=["Class 0", "Class 1"])
        plot_confusion_matrix(Results["val"]["confusion_matrix"][-1],
                              title=f"Validation Confusion Matrix - Fold {fold+1}",
                              save_path=f"Results/MicroKPNN_MT_plots/fold_{fold+1}_val_conf_matrix.png",
                              class_names=["Class 0", "Class 1"])
        plot_confusion_matrix(Results["test"]["confusion_matrix"][-1],
                              title=f"Test Confusion Matrix - Fold {fold+1}",
                              save_path=f"Results/MicroKPNN_MT_plots/fold_{fold+1}_test_conf_matrix.png",
                              class_names=["Class 0", "Class 1"])
        ...
        plt.tight_layout()
        plt.savefig(f"Results/MicroKPNN_MT_plots/fold_{fold+1}_metrics.png")
        plt.close()
    ...
    metrics_df.to_csv("Results/MicroKPNN_MT_plots/metrics_summary.csv", index=False)
    print("Metrics summary saved to 'Results/MicroKPNN_MT_plots/metrics_summary.csv'.")
