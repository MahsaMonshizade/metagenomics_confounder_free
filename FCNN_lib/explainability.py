...
def explain_model(model_paths, X):
    ...
    for fold, model_path in enumerate(model_paths):
        ...
        base_model = GAN(input_size=X.shape[1], latent_dim=32, num_layers=1).to(device)
        ...
        plt.savefig(f"Results/FCNN_plots/shap_summary_fold{fold + 1}.png")
        plt.close()
        ...
        plt.yticks(fontsize=10)               # Adjust font size for y-axis
        plt.tight_layout()                    # Ensure layout fits
        plt.savefig(f"Results/FCNN_plots/shap_bee_swarm_fold{fold + 1}.png")
        plt.close()

        # Save SHAP values as CSV
        shap_df = pd.DataFrame(shap_values_np, columns=visual_feature_columns)
        shap_df.to_csv(f"Results/FCNN_plots/shap_values_fold{fold + 1}.csv", index=False)
        print(f"SHAP values and plots saved for Fold {fold + 1}\n")

# List of trained model paths
model_paths = [
    f"Results/FCNN_plots/trained_model{fold + 1}.pth" for fold in range(5)
]
