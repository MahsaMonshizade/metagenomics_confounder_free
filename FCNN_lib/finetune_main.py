...
def main():
    ...
    os.makedirs("Results/FCNN_finetune_plots", exist_ok=True)
    ...
    pd.Series(feature_columns).to_csv("Results/FCNN_finetune_plots/feature_columns.csv", index=False)
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(X, y_all)):
        ...
        # Save model.
        torch.save(model.state_dict(), f"Results/FCNN_finetune_plots/trained_model_fold{fold+1}.pth")
        ...
        plot_confusion_matrix(Results["train"]["confusion_matrix"][-1],
                              ...
                              save_path=f"Results/FCNN_finetune_plots/fold_{fold+1}_train_conf_matrix.png",
                              class_names=["Class 0", "Class 1"])
        plot_confusion_matrix(Results["val"]["confusion_matrix"][-1],
                              title=f"Validation Confusion Matrix - Fold {fold+1}",
                              save_path=f"Results/FCNN_finetune_plots/fold_{fold+1}_val_conf_matrix.png",
                              class_names=["Class 0", "Class 1"])
        plot_confusion_matrix(Results["test"]["confusion_matrix"][-1],
                              title=f"Test Confusion Matrix - Fold {fold+1}",
                              save_path=f"Results/FCNN_finetune_plots/fold_{fold+1}_test_conf_matrix.png",
                              class_names=["Class 0", "Class 1"])
        ...
        plt.tight_layout()
        plt.savefig(f"Results/FCNN_finetune_plots/fold_{fold+1}_metrics.png")
        plt.close()
    ...
    metrics_df.to_csv("Results/FCNN_finetune_plots/metrics_summary.csv", index=False)
