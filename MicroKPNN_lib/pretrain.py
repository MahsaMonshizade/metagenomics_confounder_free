...
def train_model(model, data_all_loader, data_all_val_loader, data_all_test_loader, num_epochs, criterion_reconstructor, optimizer_reconstructor, device):
    """
    ...
    for epoch in range(num_epochs):
        ...
        epoch_val_loss = 0
        with torch.no_grad():
            for x_batch in data_all_val_loader:
                x_batch = x_batch.to(device)
        ...
        with torch.no_grad():
            for x_batch in data_all_test_loader:
                x_batch = x_batch.to(device)
                encoded_features = model.encoder(x_batch)
                reconstructed_x = model.reconstructor(encoded_features)
                rec_loss = criterion_reconstructor(reconstructed_x, x_batch)
