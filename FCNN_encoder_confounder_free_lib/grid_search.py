...
if __name__ == "__main__":
    ...
    for idx, combo in enumerate(itertools.product(*values), start=1):
        ...
        output_dir = f"Results/FCNN_encoder_confounder_free{idx}_plots"
        os.makedirs(output_dir, exist_ok=True)

        # Save hyperparameters used
        pd.DataFrame([params]).to_csv(
            os.path.join(output_dir, "hyperparameters.csv"), index=False
        )

        # Prepare config for this run
        cfg = deepcopy(config)
        # Update training and model sections
        # Training
        for tkey in ["learning_rate", "encoder_lr", "classifier_lr", "batch_size"]:
            if tkey in params:
                cfg["training"][tkey] = params[tkey]
        # Model
        for mkey in ["num_encoder_layers", "num_classifier_layers", "dropout_rate",
                    "activation", "last_activation", "latent_dim", "norm"]:
            if mkey in params:
                cfg["model"][mkey] = params[mkey]

        # Set output_dir and run
