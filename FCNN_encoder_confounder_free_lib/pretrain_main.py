...
from pretrain_models import GAN
from models import PearsonCorrelationLoss, MSEUniformLoss, KLDivergenceLoss
...
def main():
    os.makedirs("Results/FCNN_encoder_confounder_free_pretraining", exist_ok=True)
    ...
    # Get the column name for disease and confounder.
    ...
    # print(f"Training data has NaN: {has_nan_X}")
    # has_nan_X_test = np.isnan(X_test).any()
    # print(f"Test data has NaN: {has_nan_X_test}")
    ...
    # # DEBUG: Check for NaN values
    # has_nan_confounder = np.isnan(x_confounder.numpy()).any()
    # print(f"Confounder data has NaN: {has_nan_confounder}")
    # has_nan_y_confounder = np.isnan(y_confounder.numpy()).any()
    # print(f"Confounder labels have NaN: {has_nan_y_confounder}")
    ...
    torch.save(model.state_dict(), f"Results/FCNN_encoder_confounder_free_pretraining/pretrained_model.pth")
    pd.Series(feature_columns).to_csv("Results/FCNN_encoder_confounder_free_pretraining/feature_columns.csv", index=False)
    ...
    plt.tight_layout()
    plt.savefig(f"Results/FCNN_encoder_confounder_free_pretraining/pretraining_metrics.png")
    plt.close()
    ...
    metrics_df.to_csv("Results/FCNN_encoder_confounder_free_pretraining/pretraining_metrics_summary.csv", index=False)
    print("Metrics summary saved to 'Results/FCNN_encoder_confounder_free_pretraining/pretraining_metrics_summary.csv'.")
