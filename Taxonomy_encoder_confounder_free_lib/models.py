...
class GAN(nn.Module):
    ...
    """
    def __init__(self, input_size, masks, index_maps, taxonomic_ranks,  num_layers=1):
        super(GAN, self).__init__()

        self.latent_dim = len(index_maps['order'])
        self.encoder = self._build_encoder(masks, index_maps, taxonomic_ranks)
    ...
    def _build_encoder(self, masks, index_maps, taxonomic_ranks):
        ...
        for i in range(len(taxonomic_ranks) - 1):
            ...
            in_size = len(index_maps[from_rank])
            out_size = len(index_maps[to_rank])
