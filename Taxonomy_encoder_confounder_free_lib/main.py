...
def main():
    ...
    os.makedirs('Results/Taxonomy_encoder_confounder_free_plots', exist_ok=True)
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(X, y_all)):
        ...
        plot_confusion_matrix(Results['train']['confusion_matrix'][-1], 
                      ...
                      save_path=f'Results/Taxonomy_encoder_confounder_free_plots/fold_{fold+1}_train_conf_matrix.png',
                      class_names=['Class 0', 'Class 1'])

        plot_confusion_matrix(Results['val']['confusion_matrix'][-1], 
                      title=f'Validation Confusion Matrix - Fold {fold+1}', 
                      save_path=f'Results/Taxonomy_encoder_confounder_free_plots/fold_{fold+1}_val_conf_matrix.png',
                      class_names=['Class 0', 'Class 1'])

        plot_confusion_matrix(Results['test']['confusion_matrix'][-1], 
                      title=f'Test Confusion Matrix - Fold {fold+1}', 
                      save_path=f'Results/Taxonomy_encoder_confounder_free_plots/fold_{fold+1}_test_conf_matrix.png',
                      class_names=['Class 0', 'Class 1'])
        ...
        plt.tight_layout()
        plt.savefig(f'Results/Taxonomy_encoder_confounder_free_plots/fold_{fold+1}_metrics.png')
        plt.close()
    ...
    plt.tight_layout()
    plt.savefig('Results/Taxonomy_encoder_confounder_free_plots/average_metrics.png')
    plt.close()
    ...
    plot_confusion_matrix(train_avg_metrics['confusion_matrix'][-1], 
                        ...
                        save_path='Results/Taxonomy_encoder_confounder_free_plots/average_train_conf_matrix.png',
                        class_names=['Class 0', 'Class 1'])

    plot_confusion_matrix(val_avg_metrics['confusion_matrix'][-1], 
                        title='Average Validation Confusion Matrix', 
                        save_path='Results/Taxonomy_encoder_confounder_free_plots/average_val_conf_matrix.png',
                        class_names=['Class 0', 'Class 1'])

    plot_confusion_matrix(test_avg_metrics['confusion_matrix'][-1], 
                        title='Average Test Confusion Matrix', 
                        save_path='Results/Taxonomy_encoder_confounder_free_plots/average_test_conf_matrix.png',
                        class_names=['Class 0', 'Class 1'])
    ...
    # Save to CSV
    metrics_df.to_csv('Results/Taxonomy_encoder_confounder_free_plots/metrics_summary.csv', index=False)

    print("Metrics for each fold and their average have been saved to 'Results/Taxonomy_encoder_confounder_free_plots/metrics_summary.csv'.")
